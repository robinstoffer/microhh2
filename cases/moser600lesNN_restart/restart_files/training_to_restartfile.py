...
#Select flow fields for selected time step, remove all the ghost cells, and denormalize
uc_singlefield = np.array(a['uc'][t,kgc_center:kend,jgc:jend,igc:iend]) * utau_ref
vc_singlefield = np.array(a['vc'][t,kgc_center:kend,jgc:jend,igc:iend]) * utau_ref
wc_singlefield = np.array(a['wc'][t,kgc_center:kend,jgc:jend,igc:iend]) * utau_ref
